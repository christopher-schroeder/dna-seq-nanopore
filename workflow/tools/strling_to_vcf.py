...
##INFO=<ID=ALLELE1,Number=A,Type=Float,Description="Repeat length estimation for allele 1">
##INFO=<ID=ALLELE2,Number=A,Type=Float,Description="Repeat length estimation for allele 2">
##INFO=<ID=CONTROLS,Number=.,Type=Float,Description="The estimations of the controls.">
##FORMAT=<ID=AS1,Number=A,Type=Float,Description="Estimation of the first allele size.">
##FORMAT=<ID=AS2,Number=A,Type=Float,Description="Estimation of the second allele size.">
##FORMAT=<ID=ACN1,Number=A,Type=Float,Description="Estimation of the first copy number.">
##FORMAT=<ID=ACN2,Number=A,Type=Float,Description="Estimation of the second copy number.">
##FORMAT=<ID=ASP1,Number=A,Type=Float,Description="Estimation of the first support read number.">
##FORMAT=<ID=ASP2,Number=A,Type=Float,Description="Estimation of the second support read number.">"""

def convert(filename, fileoutput, samplename):
    with open(filename, "r") as f, open(fileoutput, "w") as o:
        print(header, file=o)
        for i, line in enumerate(f):
            if i == 0:
                Line = namedtuple("Line", ", ". join(line.strip().strip("#").replace(":", "_").split()))
                print("#CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", samplename, sep="\t", file=o)
                continue
            line = line.replace("-", ".")
            l = Line(*line.strip("\n").split())
            ...
            print(
                ...
                ".",
                "PASS",
                f"SVTYPE=STR;END={l.end}",
                "AS1:AS2:ACN1:ACN2:ASP1:ASP2",
                f"{l.allele1_size}:{l.allele2_size}:{l.allele2_copy_number}:{l.allele2_copy_number}:{l.allele2_support}:{l.allele2_support}",
                sep="\t", file=o)
